(()=>{
  ...
  function renderVariant(v){return `<article class="core-variant"><header><h4>${esc(v.name||v.label||v.id)}</h4><p>${esc(v.description||v.text||'')}</p></header>${v.stats?`<div class="core-stat-pills">${Object.entries(v.stats).map(([k,val])=>`<span><small>${k==='hpFormula'?'HP':k==='defense'?'DEFESA':esc(k)}</small><b>${esc(val)}</b></span>`).join('')}</div>`:''}${v.tiers?.length?renderTiers(v.tiers):''}${v.traits?.length?`<ul>${v.traits.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`:''}${v.abilities?.length?`<div class="core-variant-abilities">${v.abilities.map(a=>`<section><h5>${esc(a.name||a.label||a.id)}</h5>${a.description?`<p>${esc(a.description)}</p>`:''}${a.tiers?.length?renderTiers(a.tiers):''}</section>`).join('')}</div>`:''}</article>`;}
  function choiceCountText(choice){
    const axis=choice.progressionAxis==='characterLevel'?'nível':'estacas';
    if(choice.progression?.length){
      const parts=choice.progression.map(r=>`${r.max==null?`${r.min}+`:`${r.min}–${r.max}`}: ${r.count}`);
      return `${parts.join(' · ')} (${axis})`;
    }
    return choice.choose?`Escolha ${choice.choose}`:'';
  }
  function renderChoiceMeta(ab){
    if(!ab.choices?.length)return '';
    return `<div class="core-choice-meta">${ab.choices.map(ch=>{
      const opts=(ch.options||[]).map(o=>o.label||o.name||o.id);
      const dynamic=ch.dynamicOptions?.type==='alreadyProficientSkills'?'perícias em que já possui proficiência':ch.dynamicOptions?.type||'';
      const pool=opts.length?`<span class="core-choice-options">${opts.map(esc).join(' · ')}</span>`:(dynamic?`<span class="core-choice-options">${esc(dynamic)}</span>`:'');
      const progression=choiceCountText(ch);
      return `<section><small>ESCOLHA PERSISTENTE</small><strong>${esc(ch.label||ch.id)}</strong>${progression?`<em>${esc(progression)}</em>`:''}${pool}${ch.allowReplace===false?'<p>As escolhas registradas não podem ser trocadas depois.</p>':''}</section>`;
    }).join('')}</div>`;
  }
  function resourceMaxText(r){
    const max=r?.max||{};
    if(max.type==='fixed')return `máximo ${max.value}`;
    if(max.type==='levelFormula')return max.formula==='8 + floor(nível/10)'?'8 + 1 a cada 10 níveis':(max.formula||'');
    if(max.type==='stakeProgression'&&max.progression?.length)return max.progression.map(x=>`${x.max==null?`${x.min}+`:`${x.min}–${x.max}`}: ${x.value}`).join(' · ');
    if(max.type==='described')return max.text||'conforme a habilidade';
    return '';
  }
  function resourceScopeLabel(scope){return ({personal:'PESSOAL',collective:'COLETIVO',target:'POR ALVO',ability:'NA HABILIDADE'})[scope]||String(scope||'').toUpperCase();}
  function resourceCards(d){
    const resources=d.resources?.length?d.resources:(d.resource?[{name:d.resource.name,scope:d.resource.scope,max:{type:'described',text:d.resource.maxFormula||''}}]:[]);
    return resources.map(r=>`<article class="deity-info deity-resource"><small>RECURSO · ${esc(resourceScopeLabel(r.scope))}</small><p><strong>${esc(r.name)}</strong>${resourceMaxText(r)?` · ${esc(resourceMaxText(r))}`:''}</p></article>`).join('');
  }
